...
export default function UrgentScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-background dark:bg-background-dark">
      ...
      <FlashList data={problems} renderItem={({ item }) => <UrgentRow problem={item} onPress={() => router.push(`/urgent/${item.id}`)} />} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 20 }}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary.DEFAULT} />}
        ListEmptyComponent={!isLoading ? (<View className="items-center pt-20"><Ionicons name="alert-circle-outline" size={48} color={colors.muted.foreground} /><Text className="text-lg font-semibold text-foreground dark:text-foreground-dark mt-4">{t("urgent.noProblems")}</Text><Text className="text-muted-foreground text-sm mt-1">{t("urgent.noProblemsSubtitle")}</Text></View>) : null}
      />
    </SafeAreaView>
  );
}
